...
enum ScreenModes { COUNT, RESET, FLAG };
ScreenModes state = RESET;

void drawMeme(bool whiteout) {
    tft.setTextSize(4);
    if(whiteout) {
...
void setupScreen() {
  tft.setRotation(4); //CHANGED TO 4
  
  tft.fillScreen(WHITE);//making the background
  tft.fillRect(0,305,175,160,GREEN);
  tft.fillRect(160,305,175,160,RED);
  drawMeme(false);
}

void drawFlag(int xOffset, int yOffset) { 
  tft.fillRect(5,0,20,480,DARK_GREY);
...
void timer() { //change to bool or enum
...
int getTouchInput() {
  //TOUCH INPUT BUTTON CHECKING//
  TSPoint p = ts.getPoint(); // read in what the touch screen is reading
  if(p.z > MINPRESSURE && p.z < MAXPRESSURE) { 
    
    //PROCESSING TOUCH INPUT//
    p.x = p.x + p.y; //touch coordinate scailing from the tftpaint example
    p.y = p.x - p.y; //the touch coordinates are alligned with tft.setRotation(4);           
    p.x = p.x - p.y;   
    p.x = map(p.x, TS_MINX, TS_MAXX, tft.width(), 0);
    p.y = tft.height()-(map(p.y, TS_MINY, TS_MAXY, tft.height(), 0));
    //ONLY OCCURS ON SUFFICIENT TOUCH PRESSURE//
...
void setup(void) {
  //Serial.begin(9600); //for debug on the computer
  Serial1.begin(9600);
  //Serial1.println("Screen starting up..."); //DEBUG
...
void loop(void) {
  ...
  if(state == COUNT) {
    if(countdown < 5) state = FLAG; 
    //Serial.print("Flag\n");
  }

  switch(state) {
    case FLAG:
      tft.fillRect(0,0,175,320,WHITE); //all this stuff is here to make screen clearing faster--the micro seems to struggle writing lots of pixles
      tft.setCursor(0,265);
      tft.setTextColor(WHITE);
      tft.print("Timer: 0: 5");
    ...
    case RESET:
      ...
      //nothing happens in this state, we are idle
      break;
  }
...
///////////////////////////////////////////
  ...
  //TIMER//
  timer(); //Method which counts down every second
