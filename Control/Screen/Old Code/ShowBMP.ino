...
const int __Gnbmp_height = 211;                 // Initially 480
...
char __Gsbmp_files[1][FILENAME_LEN] =           // add file name here, make the first array bount equal to __Gnfile_num (equal to the amount of files you have
{
"01.bmp",
/*"02.bmp",
"03.bmp",
...
void loop(void) {
  tft.fillRect(300,0,20,480,WHITE);
     for(unsigned char i=0; i<__Gnfile_num; i++) {
        ...
        //
        while(1) {
          if (++scroll >= 10) while(1){}; //scroll = 0
          tft.vertScroll(0,480,(scroll)*-30); 
          //vertScroll summary
          //1st: The lowest point that the screen's image is pulled down from
          //2nd: The highest point on the screen where the image is moved down from, highest is 480
          //3rd: scroll rate, smaller values scroll a smaller amount per cycle (offset)
          delay(100);
        }
        //
    }
